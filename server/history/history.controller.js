...
exports.historyAdmin = async (req, res) => {
  try {
    ...
    if (req.query.type === "call" || req.query.type === "message" || req.query.type === "randomMatch" || req.query.type === "gift") {
      ...
      return res.status(200).json({
        ...
        totalCoin: history[0].callCharge.length > 0 ? history[0].callCharge[0].total : 0,
      ...
      return res.status(200).json({
        ...
        total: history[0].pageInfo.length > 0 ? history[0].pageInfo[0].totalRecord : 0,
        totalCoin: history[0].totalCoin.length > 0 ? history[0].totalCoin[0].totalCoin : 0,
      });
    } else if (req.query.type === "coinPlan") {
      const history = await History.aggregate([
...
exports.adminCoinHistory = async (req, res) => {
  try {
    ...
    if (req.query.userId) {
      ...
      matchQuery = {
        $and: [{ userId: { $eq: user._id } }, { isIncome: { $eq: true } }, { type: { $eq: 8 } }],
      };
      ...
      matchQuery = {
        $and: [{ hostId: { $eq: user._id } }, { isIncome: { $eq: true } }, { type: { $eq: 8 } }],
      };
...
exports.makeCall = async (req, res) => {
  try {
    ...
    if (!req.body || !req.body.callerId || !req.body.receiverId || !req.body.videoCallType || !req.body.statusType) {
      return res.status(200).json({ status: false, message: "Oops ! Invalid details." });
    }
    ...
    const caller = req.body.videoCallType === "user" ? user : host;

    if (req.body.statusType === "random") {
      if (user.coin < settingJSON.chargeForRandomCall) {
        console.log("you don't have sufficient coin.");

        return res.status(200).json({ status: false, message: "User don't have sufficient coins." });
      }
    } else if (req.body.statusType === "normal") {
      if (user.coin < settingJSON.chargeForPrivateCall) {
        console.log("you don't have sufficient coin.");

        return res.status(200).json({ status: false, message: "User don't have sufficient coins." });
      }
    } else {
      return res.status(200).json({ status: false, message: "StatusType is invalid." });
    }

    if (req.body.statusType !== "live") {
      if ((host.isBusy && req.body.videoCallType === "user") || (req.body.videoCallType === "host" && user.isBusy)) {
        return res.status(200).json({
          status: false,
    ...
    outgoing.callStartTime = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    ...
    income.callStartTime = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    ...
    await Promise.all([host.save(), user.save(), outgoing.save(), income.save()]);
