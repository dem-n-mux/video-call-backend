...
exports.update = async (req, res) => {
  try {
    ...
    coinPlan.platFormType = req.body.platFormType ? parseInt(req.body.platFormType) : parseInt(coinPlan.platFormType);
    await coinPlan.save();
...
exports.createHistory = async (req, res) => {
  try {
    if (req.body.userId && req.body.coinPlanId && req.body.paymentGateway) {
      ...
      if (!coinPlan) {
        return res.json({ status: false, message: "coinPlanId does not exist!!" });
      }
