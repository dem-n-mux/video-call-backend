...
router.patch("/updateHost", upload.fields([{ name: "album" }, { name: "image", maxCount: 1 }]), checkAccessWithSecretKey(), hostFile.updateHostProfile);

//Update host In App
router.patch("/updateProfile", upload.fields([{ name: "album" }, { name: "image", maxCount: 1 }]), checkAccessWithSecretKey(), hostFile.updateProfile);

//Update Host Image, CoverImage and Album [App]
router.patch("/updateImage", upload.fields([{ name: "album" }, { name: "image", maxCount: 1 }, { name: "coverImage", maxCount: 1 }]), checkAccessWithSecretKey(), hostFile.updateHostImage);
...
router.post("/AddFakehost", checkAccessWithSecretKey(), upload.fields([{ name: "image" }, { name: "video" }, { name: "album" }]), hostFile.AddFakeHost);

//update Fake host by admin
router.patch("/updateFakeHost", checkAccessWithSecretKey(), upload.fields([{ name: "image" }, { name: "video" }, { name: "album" }]), hostFile.updateFakeHost);
...
router.patch("/isBusy", checkAccessWithSecretKey(), hostFile.isBusy);

//delete user account
router.delete("/deleteHostAccount", checkAccessWithSecretKey(), hostFile.deleteHostAccount);
