  ...
  try {
    ...
    if (!settingJSON?.isFake) {
      const [user, host, chatTopic] = await Promise.all([
        ...
        }).populate("hostId", "isFake videoType video"),
      ]);
      ...
      const data = await ChatTopic.findById(newChatTopic._id).populate("hostId", "isFake videoType video");
      ...
      console.log("=======isFake=======", true);

      const [user, host, chatTopic, fakeChatTopic] = await Promise.all([
        User.findById(userId),
        Host.findById(hostId),
        ChatTopic.findOne({
          $and: [{ userId: userId }, { hostId: hostId }],
      ...
      if (fakeChatTopic) {
        return res.status(200).json({ status: true, message: "Success", chatTopic: fakeChatTopic });
      }

      const newChatTopic = new ChatTopic();

      newChatTopic.userId = user?._id || null;
      newChatTopic.hostId = host._id;
...
exports.getChatThumbList = async (req, res) => {
  try {
    if (!req.query.type || !req.query.userId) {
      return res.status(200).json({ status: false, message: "Type and UserId must be required!!" });
    }
    ...
    if (req.query.type === "user") {
      ...
      const blockUser = await Block.find({ hostId: user._id }).distinct("userId");
    ...
    const [list, fakeChatList] = await Promise.all([
      ChatTopic.aggregate([
        {
          $match: matchQuery,
        },
        ...
        {
          $facet: {
            chatList: [{ $skip: req.query.start ? parseInt(req.query.start) : 0 }, { $limit: req.query.limit ? parseInt(req.query.limit) : 20 }],
          },
        },
      ]),
      ...
      Host.aggregate([
        ...
        },
        {
          $addFields: {
            ...
            topic: "",
            message: {
              $arrayElemAt: [datingMessages, Math.floor(Math.random() * datingMessages.length)],
            },
            date: "",
            chatDate: "",
            time: "",
...
exports.userSearch = async (req, res) => {
  try {
    ...
    if (req.body.name) {
      req.body.name === "@#"
        ? (response = await Host.find({ isBlock: false }).skip(start).limit(limit))
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        if (chatTopic) {
          ...
          if (chat) {
            time =
              now.diff(chat.createdAt, "minute") <= 60 && now.diff(chat.createdAt, "minute") >= 0
